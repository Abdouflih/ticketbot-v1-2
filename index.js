...
const db = new Database("mongodb+srv://yassinehzz:GiLjXZaSfaBA994@cluster0.j0u4f.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
...
client.on('ready', async () => {
  ...
  console.log(`Credits | Einstein#1210`)
...
> \`${prefix}setstaff\` - set staff roles`)
  ...
  if (command == prefix + 'send' || command == prefix + 'ticket') {
    ...
    let args = message.content.split(' ').slice(1).join(' ');
    if (!args) args = `**Tickets support/ Application / Partnership**`
    ...
    .setLabel('Wanna Design Your Discord Server')
    .setEmoji('898132754615660554')
    .setValue("des")
    .setDescription('Use this to contact Helpers and higher ranks!')
    let button2 = new MessageMenuOption()
    .setLabel('التـقديم للادارة')
    ...
    .setLabel('Want any additionl Api key')
    ...
    .setMaxValues(1)
    .setMinValues(1)
    .addOptions(button1, button2)
    let embed = new Discord.MessageEmbed()
      .setTitle(args) 
      .setDescription("**__To create a ticket__**, select one of the options below from the menu \n **If you need help, want to apply or if you are having __Questions__, then please Open a Ticket**")
...
client.on('clickMenu', async (button) => {
  ...
  if (await db.get(`tickets_${button.id}_${button.message.guild.id}`)) {
    ...
    if (button.values[0] === "men") { //admin +
          button.guild.channels.create(`ticket-${count}`, {
            ...
            ], parent: (await db.get(`Channels_${button.message.guild.id}.Cat`)), position: 1, topic: `A Ticket : <@!${button.clicker.user.id}>`, reason: ""
          }).then(async channel => {
            channel = channel
      ...
      **Your ticket has been successfully opened** <#${channel.id}>`, true)
      ...
      To close this ticket, interact with 🔒`)
            ...
            let bu1tton = new disbut.MessageButton()
              ...
              .setID(idd)
            channel.send(`Welcome <@!${button.clicker.user.id}> wait for <@&858682515351011368> `, { embed: embedticket, component: bu1tton }).then(msg => {
              msg.pin()
            })
            })
        }
      ...
      **Your ticket has been successfully opened** <#${channel.id}>`, true)
      ...
      To close this ticket, interact with 🔒`)
            ...
            let bu1tton = new disbut.MessageButton()
              ...
              .setLabel(`Close ticket`)
              .setID(idd)
            channel.send(`Welcome <@!${button.clicker.user.id}> <@&858682515351011368> `, { embed: embedticket, component: bu1tton }).then(msg => {
              msg.pin()
            })
            })
        }
 ...
 if (button.values[0] === "des"){ // design +
      ...
      **Your ticket has been successfully opened** <#${channel.id}>`, true)
            ...
            const embedticket = new Discord.MessageEmbed()
              .setTimestamp()
              .setTitle("شروط التقديم اكتب:\n خبرات:\n مدة التفاعل:\n هل يوجد سيرفرات  مشهورة كنت اداري فيها:\nاسمك:\nعمرك:\n بلدك:\n")
              .setFooter(`Ticket opened at`)
              .setColor(`#33cd15`)
              .setDescription(`Support will be with you soon.\n
      To close this ticket, interact with 🔒`)
